...
def resize_and_save_image(image_path: str, output_path: str, width: int, height: int, distance: float):
      original = Image.open(image_path)
      o_width, o_height = original.size
      aspect_ratio = o_width / o_height
      
      if o_width > o_height:
        new_width = round(distance * 1024)
        new_height = round(distance * new_width / aspect_ratio)
      else:
        new_height = round(distance * 1024)
    ...
    # Headshot poses are not coming out consistently good
      ...
      for pose in poses:
        if width and height:
            ...
            resize_and_save_image(pose["kps"], resized_kps_path, width, height, distance)
