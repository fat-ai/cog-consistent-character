...
class Predictor(BasePredictor):
    ...
    # Calculate the position to center the input image
      ...
      position = ((width - input_width) // 2, (height - input_height) // 2)
    
    # Paste the input image onto the black background
      background.paste(input_image, position)
    ...
    width: int = Input(
        description="The width of the output images.", default=None
    ),
    height: int = Input(
        description="The height of the output images.", default=None
    ),
    ...
    self.handle_input_file(subject, filename="subject.png", width=width, height=height)
